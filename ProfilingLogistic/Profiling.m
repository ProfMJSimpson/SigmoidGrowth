...
%% MLE
funmle=@(n) - Logistic_Likelihood(n(1), n(2), n(3), n(4), t, Ndata);
...
for i = 1:numel(rrange)
    i
rr=rrange(i);
funr=@(n) - Logistic_Likelihood(rr, n(1), n(2), n(3), t, Ndata);
...
%problem = createOptimProblem('fmincon','x0',[K0,N00,sigma0],'objective',funr,'lb',[1e-10,1e-10,1e-10],'ub',[]);
%[nrange(:,i)] = run(gs,problem);
end


for i=1:numel(rrange)
lhoodr(i)=-Logistic_Likelihood(rrange(1,i),nrange(1,i),nrange(2,i),nrange(3,i),t,Ndata); 
end
...
for i = 1:numel(krange)
    i
kk=krange(i);
funk=@(n) -Logistic_Likelihood(n(1), kk, n(2), n(3), t, Ndata);
...
[nrange(:,i)] = fmincon(funk,n0,[],[],[],[],([0,0,0]),([1,100,1e8]));
%problem = createOptimProblem('fmincon','x0',[lambda0,N00,sigma0],'objective',funk,'lb',[1e-10,1e-10,1e-10],'ub',[]);
%[nrange(:,i)] = run(gs,problem);
end


for i=1:numel(krange)
lhoodk(i)=-Logistic_Likelihood(nrange(1,i),krange(1,i),nrange(2,i),nrange(3,i),t,Ndata); 
end
...
for i = numel(N0range):-1:1
N0=N0range(i);
i
funN0=@(n) - Logistic_Likelihood(n(1), n(2), N0, n(3), t, Ndata);


      if i==numel(N0range)
      %n0=[mle(1,1), mle(1,2), mle(1,4)];
...
lhoodN0(i)=-Logistic_Likelihood(nrange(1,i),nrange(2,i),N0range(1,i),nrange(3,i),t,Ndata); 
end
...
sg=sigmarange(i);
i
funsigma=@(n) - Logistic_Likelihood(n(1), n(2), n(3), sg, t, Ndata);
...
lhoodsigma(i)=-Logistic_Likelihood(nrange(1,i),nrange(2,i),nrange(3,i),sigmarange(1,i),t,Ndata); 
end
