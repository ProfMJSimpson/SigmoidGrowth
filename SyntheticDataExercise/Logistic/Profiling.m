...
clc
rng(1);
%bb= linspace(0.05,1.5,30);
bb=1.00
% Line 4 use to profile across a range of beta values
% Line 5 just to profile one value of beta and generate plots
npts=100;
TH=-1.9207;
...
subplot(2,1,1)
plot(t*lambda*beta,Ndata/K,'r','LineWidth',1)
Ndata=Ndata+normrnd(0,sigma,[1,length(t)]);
hold on
plot(t*lambda*beta,Ndata/K,'o','MarkerFaceColor','b','MarkerEdgeColor','b','MarkerSize',2)
 %ylim([0 1.2])
 xlim([0 tt*lambda*beta])
...
subplot(2,1,2)
plot(t*lambda*beta,res,'o','MarkerFaceColor','b','MarkerEdgeColor','b')
...
plot(rrange,lhood2r,'b','LineWidth',2)
hold on
xline(mle(1,1),'r')
xline(beta*lambda,'c')
ylim([-3, 0])
...
figure
subplot(1,2,1)
plot(bb,err,'ob','MarkerFaceColor','b','MarkerEdgeColor','b')
xlabel('\beta')
ylabel('\beta \lambda_3 - \lambda_1')
subplot(1,2,2)
plot(bb,width,'ob','MarkerFaceColor','b','MarkerEdgeColor','b')
xlabel('\beta')
ylabel('CI Width')
